...
const validateName = (name) => {
  ...
  if (isNaN(fechaInicio) || isNaN(fechaTermino)) return false;
  ...
  const validateDescripcion = (desc) => {
    return desc.trim().length > 0 && desc.length <= 500;
  };
  ...
  document.getElementById("form-actividades").addEventListener("submit", (e) => {
    ...
    let idRedSocial = document.getElementById('id-red-social').value;
    ...
    if (!validateDescripcion(descripcion)) errores.push("Descripción (obligatoria, máx. 500 caracteres)");
    if (!validateGlosaOtro(temaOtro)) errores.push("La descripción del tema 'otro' debe tener máximo 15 caracteres");
